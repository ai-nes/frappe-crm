...
def execute():
	...
	for name in frappe.get_all("CRM View Settings", filters={"dt": "CRM Organization"}, pluck="name"):
	...
	for view in frappe.get_all(
		...
		if rows and stale_row_fields.intersection(rows):
			frappe.delete_doc("CRM View Settings", view.name, force=True)
			continue

		if view.route_name in ("Organizations", None, ""):
			frappe.db.set_value("CRM View Settings", view.name, "route_name", "High Schools")
