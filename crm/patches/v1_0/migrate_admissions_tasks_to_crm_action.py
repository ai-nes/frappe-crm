...
def execute():
	...
	for row in frappe.get_all("Task", fields="*"):
		...
		if not student or not frappe.db.exists("CRM Student", student):
			...
			continue
		if frappe.db.exists("CRM Action", {"legacy_generic_task": row.name}):
			continue
		...
		doc = frappe.get_doc({"doctype": "CRM Action", "legacy_generic_task": row.name, "student": student, "contact": reference_docname if reference_doctype == "CRM Contact" else frappe.db.get_value("CRM Contact", {"student": student}, "name"), "origin": "manual", "action_type": _TYPE.get(str(row.get("title") or "").split(" ", 1)[0].lower()), "objective": str(row.get("description") or row.get("title") or "Migrated admissions action")[:500], "disposition": "ACT" if _TYPE.get(str(row.get("title") or "").split(" ", 1)[0].lower()) else "MONITOR", "state": status, "execution_status": "completed" if status == "completed" else "planned", "priority": priority if priority in {"high", "medium", "low"} else "medium", "due_at": row.get("due_date"), "action_owner": staff, "source_context_revision": int(frappe.db.get_value("CRM Student", student, "student_context_revision") or 0), "policy_context_version": "task-cutover-v1", "generation_idempotency_key": f"generic-task:{row.name}", "producer_identity": "frappe:migration", "payload_digest": hashlib.sha256(row.name.encode()).hexdigest(), "created_at": row.get("creation"), "completed_at": row.get("modified") if status == "completed" else None, "action_revision": 1, "decision_revision": 1})
