...
from crm.patches.v1_0 import apply_phase2_role_policy, setup_crm_permissions
...
class TestApplyPhase2RolePolicy(FrappeTestCase):
	def test_forward_patch_is_registered_after_the_existing_role_contract_patch(self):
		...
		self.assertLess(
			...
			patches.index("crm.patches.v1_0.apply_phase2_role_policy"),
		)

	def test_forward_patch_uses_only_database_adapters(self):
		with (
			patch.object(apply_phase2_role_policy, "create_roles") as create_roles,
			patch.object(apply_phase2_role_policy, "apply_managed_docperms") as apply_docperms,
			patch.object(apply_phase2_role_policy.frappe, "clear_cache") as clear_cache,
		):
			apply_phase2_role_policy.execute()
	...
	def test_forward_patch_rolls_back_its_database_adapter_on_failure(self):
		with (
			patch.object(apply_phase2_role_policy, "create_roles"),
			patch.object(apply_phase2_role_policy, "apply_managed_docperms", side_effect=RuntimeError("boom")),
			patch.object(apply_phase2_role_policy.frappe.db, "savepoint") as savepoint,
			patch.object(apply_phase2_role_policy.frappe.db, "rollback") as rollback,
		):
