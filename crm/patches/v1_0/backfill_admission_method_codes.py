...
def execute() -> None:
	if not frappe.db.table_exists("CRM Student") or not frappe.db.exists(
		"DocType", "CRM Admission Method"
	):
		return
	for row in frappe.db.get_all(
		"CRM Student", fields=["name", "admission_method"], limit_page_length=0
	):
		...
		if code and frappe.db.exists("CRM Admission Method", code):
			frappe.db.set_value(
				"CRM Student", row["name"], "admission_method", code, update_modified=False
			)
