...
class TestDirectorAnalytics(FrappeTestCase):
	def _policy(self):
		return frappe._dict(actor="director@example.com", profile="admissions_director", revision="policy", scope_version="campus-a", campuses=("Campus A",), capabilities=frozenset({"admissions.oversee"}))
	...
	def test_every_director_menu_route_has_a_closed_definition_and_readiness(self):
		for menu_id, readiness in director_analytics.DIRECTOR_ROUTE_READINESS.items():
			self.assertIn((readiness["workspace"], readiness["view"]), director_analytics.DIRECTOR_VIEW_DEFINITIONS, menu_id)
	...
	def test_director_filters_are_view_specific_and_campus_cannot_expand_scope(self):
		policy = self._policy()
		self.assertEqual(normalize_filters("director-overview", {"campus": "Campus A"}, view="overview", policy=policy), {"campus": "Campus A"})
		...
		self.assertEqual(response["filterSchema"][0], {"key": "period", "label": "Period", "type": "select", "options": [], "default": None})
	...
	def test_director_snapshot_binds_definition_and_timezone(self):
		policy = self._policy()
		context = {"definitionVersion": director_analytics.DIRECTOR_DEFINITION_VERSION, "timezone": "Asia/Ho_Chi_Minh", "asOf": "2026-08-28 00:00:00", "watermarks": {"CRM Student": "2026-08-28 00:00:00"}}
		with patch("crm.api.role_workspaces.get_encryption_key", return_value="test-key"):
			snapshot = role_workspaces._mint_snapshot(policy, "director-overview", "overview", {}, context=context)
			payload = role_workspaces._validate_snapshot(snapshot, policy, "director-overview", "overview", {})
	...
	def test_one_director_snapshot_validates_for_summary_series_and_rows(self):
		...
		with patch("crm.api.role_workspaces.get_encryption_key", return_value="test-key"):
			...
			for endpoint in ("summary", "series", "rows"):
				self.assertEqual(role_workspaces._validate_snapshot(snapshot, policy, "director-overview", "overview", {})["asOf"], context["asOf"], endpoint)

	def test_break_glass_requires_a_distinct_entitlement(self):
		with self.assertRaises(WorkspacePolicyError):
			authorize_director_view(self._policy(), "approvals", "break-glass")
		authorize_director_view(frappe._dict({**self._policy(), "capabilities": frozenset({"governance.break_glass.read"})}), "approvals", "break-glass")

	def test_overview_uses_only_scoped_student_projection(self):
		policy = self._policy()
		with patch.object(director_analytics.frappe.db, "table_exists", return_value=True), patch.object(director_analytics.frappe.db, "count", return_value=7) as count, patch.object(director_analytics.frappe.db, "get_all", return_value=[frappe._dict(lifecycle_stage="Enrolled", value=2)]), patch.object(director_analytics.frappe.db, "get_value", return_value="2026-08-28 00:00:00"):
		...
		self.assertEqual(response["kpis"][0]["value"], 7)
		self.assertEqual(response["kpis"][1]["value"], 2)
		self.assertEqual(count.call_args.kwargs["filters"], {"branch": ["in", ["Campus A"]]})

	def test_records_do_not_fall_back_to_contact_rows(self):
		policy = self._policy()
		student = frappe._dict(name="STU-1", student_name="A", branch="Campus A", major=None, lifecycle_stage="Lead", owner_staff=None, owning_team=None, modified="2026-08-28")
		with patch.object(director_analytics.frappe.db, "table_exists", return_value=True), patch.object(director_analytics.frappe.db, "get_all", return_value=[student]), patch.object(director_analytics.frappe.db, "get_value", return_value="2026-08-28"):
		...
		self.assertEqual(response["rows"][0]["drillDown"]["resolver"], "crm.api.role_workspaces.resolve_workspace_row_detail")
		self.assertNotIn("name", response["rows"][0])

	def test_small_aggregate_is_suppressed_and_row_token_is_scope_bound(self):
		policy = self._policy()
		with patch.object(director_analytics.frappe.db, "table_exists", return_value=True), patch.object(director_analytics.frappe.db, "count", return_value=2), patch.object(director_analytics.frappe.db, "get_all", return_value=[frappe._dict(lifecycle_stage="Enrolled", value=2)]), patch.object(director_analytics.frappe.db, "get_value", return_value="2026-08-28"):
			response = director_analytics.get_summary(policy, "director-overview", "overview", {}, "snapshot")
		...
		with self.assertRaises(frappe.PermissionError):
			director_analytics.validate_row_detail_token(token, frappe._dict({**policy, "scope_version": "other"}), "snapshot")
