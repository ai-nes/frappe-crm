...
def _journal(student: str) -> dict:
	...
	# `get_list` keeps the caller's own CRM Interaction permission; the
	# CRM Student read check alone must not materialize per-source rows.
	try:
		rows = frappe.get_list("CRM Interaction", filters={"student": student}, fields=["name", "interaction_datetime", "channel", "direction", "actor", "summary", "outcome"], order_by="interaction_datetime desc, creation desc", limit_page_length=20)
	except frappe.PermissionError:
		return {"as_of": _iso(now_datetime()), **groups, "next_cursor": None, "omitted_reason": "permission_denied"}
	for row in rows:
