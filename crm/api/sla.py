...
def recompute_sla_statuses():
	...
	frappe.db.sql(
		...
		where sla_started_at is not null
		  ...
		  and timestampdiff(minute, sla_started_at, coalesce(first_contact_time, now())) >= %(breach_min)s
		""",
		{"breach": BREACH, "breach_min": SLA_BREACH_MINUTES},
	)
	frappe.db.sql(
		f"""
		update `tabCRM Contact`
		...
		where sla_started_at is not null
		  and {_OPEN_STATUS_SUBQUERY}
		  and timestampdiff(minute, sla_started_at, coalesce(first_contact_time, now())) >= %(warning_min)s
		  and timestampdiff(minute, sla_started_at, coalesce(first_contact_time, now())) < %(breach_min)s
		""",
		{"warning": WARNING, "warning_min": SLA_WARNING_MINUTES, "breach_min": SLA_BREACH_MINUTES},
	)
	frappe.db.sql(
		f"""
		update `tabCRM Contact`
