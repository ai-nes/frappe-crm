...
class TestNbaEvaluationProducer(FrappeTestCase):
	...
	def test_production_shaped_projection_produces_stable_mapped_envelope(self):
		...
		action = first["eligible_action_set"]["actions"][0]
		self.assertTrue(action["in_candidate_set"])
		self.assertTrue(action["enabled"])
		self.assertTrue(action["effective"])
		self.assertTrue(action["actor_allowed"])
		self.assertTrue(action["academic_eligible"])
		self.assertTrue(action["duplicate_active"])
		self.assertTrue(action["time_allowed"])
