...
def _record(
	*,
	...
	try:
		...
		result = {
			"status": "created",
			"evidence": {"doctype": doctype, "name": doc.name},
			"student": student,
...
@frappe.whitelist()
def record_campaign_touchpoint(
	student,
	crm_campaign,
	crm_contact=None,
	touched_at=None,
	...
	return _record(
		...
		values={
			...
			"reference_name": crm_campaign,
			"crm_campaign": crm_campaign,
			"touched_at": touched_at or frappe.utils.now_datetime(),
...
def record_event_participation(
	...
	return _record(
		...
		values={
			...
			"reference_name": crm_event,
			"crm_event": crm_event,
			"status": status or "Registered",
			"registered_at": registered_at or frappe.utils.now_datetime(),
...
def _redacted_read_authorized():
	...
	if (
		...
		and CAPABILITY not in capabilities_for_roles(roles)
	):
...
def _metrics(rows):
	...
	for row in frappe.db.get_all(
		"CRM Student", filters={"name": ["in", list(students)]}, fields=["student_stage"]
	):
...
def _student_metrics_for(kind, field, value):
	rows = frappe.db.get_all(
		...
		filters={"engagement_kind": kind, field: value},
		fields=["name", "student"],
	)
...
def get_campaign_attribution(crm_campaign, limit=100, cursor=None):
	...
	rows = frappe.db.get_all(
		"CRM Marketing Engagement",
		filters={"engagement_kind": "campaign_touch", "crm_campaign": crm_campaign},
		fields=["name", "student", "touched_at", "source", "supersedes", "creation"],
	)
	...
	return {
		"metrics": _student_metrics_for("campaign_touch", "crm_campaign", crm_campaign),
		"timeline": [
			{
				"name": row.name,
				"student": row.student,
				...
				"source": row.source,
				"superseded": row.name in superseded,
			}
			for row in page
		],
...
def get_event_attribution(crm_event, limit=100, cursor=None):
	...
	rows = frappe.db.get_all(
		"CRM Marketing Engagement",
		filters={"engagement_kind": "event_participation", "crm_event": crm_event},
		fields=["name", "student", "registered_at", "status", "supersedes", "creation"],
	)
	...
	superseded = _superseded_page_names("CRM Marketing Engagement", page)
	return {
		"metrics": _student_metrics_for("event_participation", "crm_event", crm_event),
		"timeline": [
			{
				"name": row.name,
				"student": row.student,
