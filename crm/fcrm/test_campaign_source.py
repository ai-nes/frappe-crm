...
class TestCampaignSource(FrappeTestCase):
	...
	def test_student_inherits_campaign_from_source_lead(self):
		doc = Mock()
		doc.get.side_effect = lambda fieldname: {"source_lead": "Lead 1"}.get(fieldname)
		with (
			patch.object(frappe.db, "get_value", return_value="Campaign 1"),
		...
		doc.set.assert_any_call("campaign", "Campaign 1")
