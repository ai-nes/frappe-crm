...
ACTIVE_AUTHORING_FLAG = "crm_rule_active_authoring"
...
class CRMRule(Document):
	...
	def validate(self):
		...
		active_authoring = getattr(frappe.flags, ACTIVE_AUTHORING_FLAG, False)
		if version_status != "draft" and not (version_status == "active" and active_authoring):
			frappe.throw("Rules can only be edited in a draft CRM Rule Version.", frappe.PermissionError)
		if version_status == "active" and not getattr(frappe.flags, AUTHORING_FLAG, False):
			frappe.throw(
				"Active CRM Rules can only be changed through the rule admin API.", frappe.PermissionError
			)
		try:
		...
		if before and not getattr(frappe.flags, AUTHORING_FLAG, False):
			...
			if any(before.get(fieldname) != self.get(fieldname) for fieldname in managed_fields):
				frappe.throw(
					"CRM Rules can only be changed through the rule admin API.", frappe.PermissionError
				)
		...
		if self.is_new():
			...
			if not getattr(frappe.flags, "crm_rule_lifecycle", False):
				frappe.throw(
					"CRM Rule lifecycle changes must use the rule admin API.", frappe.PermissionError
				)

		if (
			before
			and int(self.revision or 0) != int(before.revision or 0)
			and not getattr(frappe.flags, AUTHORING_FLAG, False)
		):
			frappe.throw(
				"CRM Rule revision can only change through the rule admin API.", frappe.PermissionError
			)
	...
	def on_trash(self):
		...
		version_status = str(version_status or "").lower()
		is_draft_rule = self.status == "draft" and version_status == "draft"
		is_authorized_active_rule = (
			self.status == "active"
			and version_status == "active"
			and getattr(frappe.flags, ACTIVE_AUTHORING_FLAG, False)
		)
		if not (is_draft_rule or is_authorized_active_rule):
			frappe.throw(
