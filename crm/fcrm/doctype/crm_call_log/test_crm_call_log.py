...
class TestCRMCallLog(FrappeTestCase):
	...
	def test_call_log_status_filtering(self):
		...
		completed_names = [
			log.name for log in frappe.get_all("CRM Call Log", filters={"status": "Completed"})
		]
		failed_names = [log.name for log in frappe.get_all("CRM Call Log", filters={"status": "Failed"})]
