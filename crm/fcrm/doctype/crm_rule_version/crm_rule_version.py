...
ACTIVE_AUTHORING_FLAG = "crm_rule_version_active_authoring"
	...
	def validate(self):
		...
		if self.is_new():
			...
			if data["status"] != "draft" or data["revision"]:
				frappe.throw(
					"A new CRM Rule Version must start as a draft at revision zero.", frappe.ValidationError
				)
			...
			if previous_status in {"testing", "active", "archived"} and changed and not lifecycle:
				if previous_status == "active" and active_authoring:
					protected_fields = ("version_name", "description", "schema_version")
					if any(before.get(fieldname) != self.get(fieldname) for fieldname in protected_fields):
						frappe.throw(
							"Active CRM Rule Version metadata can only be changed through its lifecycle API.",
							frappe.PermissionError,
						)
				else:
					frappe.throw(
		...
		if (
			data["status"] == "draft"
			and self.get("ruleset_digest")
			and not getattr(frappe.flags, "crm_rule_version_authoring", False)
		):
