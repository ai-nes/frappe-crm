...
def create(view: dict):
	...
	doc = frappe.new_doc("View Settings")
	doc.name = view.label
...
def update(view: dict):
	...
	doc = frappe.get_doc("View Settings", view.name)
...
def delete(name: str | int):
	if frappe.db.exists("View Settings", name):
		doc = frappe.get_doc("View Settings", name)
		check_permission(doc)
		frappe.delete_doc("View Settings", name)


@frappe.whitelist()
def public(name: str | int, value: bool | int):
	if frappe.session.user != "Administrator" and "Lead Sales" not in frappe.get_roles():
...
def check_permission(doc):
	...
	If view is public, Lead Sales can edit.
	If view is private, only the view owner can edit."""
	if frappe.session.user == "Administrator" or "System Manager" in frappe.get_roles():
		pass
	elif doc.public and "Lead Sales" in frappe.get_roles():
		pass
...
def set_as_default(name: str | int | None = None, type: str | None = None, doctype: str | None = None):
	if name:
		frappe.db.set_value("View Settings", name, "is_default", 1)
	else:
...
def create_or_update_standard_view(view: dict):
	...
	if doc:
		doc = frappe.get_doc("View Settings", doc)
		doc.label = view.label
		doc.type = view.type or "list"
		doc.route_name = view.route_name or get_route_name(view.doctype)
		doc.load_default_columns = view.load_default_columns or False
