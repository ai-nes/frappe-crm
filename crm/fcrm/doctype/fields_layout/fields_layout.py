...
def get_fields_layout(doctype: str, type: str, parent_doctype: str | None = None):
	...
	if frappe.db.exists("Fields Layout", {"dt": doctype, "type": type}):
		layout = frappe.get_doc("Fields Layout", {"dt": doctype, "type": type})
...
def get_sidepanel_sections(doctype: str):
	if not frappe.db.exists("Fields Layout", {"dt": doctype, "type": "Side Panel"}):
		return []
	layout = frappe.get_doc("Fields Layout", {"dt": doctype, "type": "Side Panel"}).layout
...
def save_fields_layout(doctype: str, type: str, layout: str):
	if not frappe.has_permission("Fields Layout", "write"):
		frappe.throw(_("Not permitted to modify fields layout"), frappe.PermissionError)

	if frappe.db.exists("Fields Layout", {"dt": doctype, "type": type}):
		doc = frappe.get_doc("Fields Layout", {"dt": doctype, "type": type})
	else:
