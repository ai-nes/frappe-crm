...
REQUEST_TYPES = frozenset(
	{"access", "rectification", "erasure", "restriction", "objection", "withdraw_consent"}
)
...
class CRMStudentPrivacyRequest(Document):
	...
	def validate(self):
		...
		if self.status == "completed" and (
			not self.resolution or not self.resolved_at or not self.resolved_by
		):
			frappe.throw(
				_("Completed privacy requests require resolution evidence and actor."), frappe.ValidationError
			)
		if self.is_new():
		...
		if self.status != before.status and not self._from_command():
			frappe.throw(
				_("Privacy request state changes must use the privacy command."), frappe.PermissionError
			)
	...
	if permission_type in {"create", "write", "delete"}:
		...
		return bool(
			...
			or (
				...
				and frappe.has_permission("CRM Student", "write", student, user=user)
			)
		)
...
def get_permission_query_conditions(user=None):
	...
	if condition == "1=0":
		return "1=0"
	return "`tabCRM Student Privacy Request`.student in (select `tabCRM Student`.name from `tabCRM Student` where ({0}))".format(
		condition
	)
