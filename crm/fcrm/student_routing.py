...
def _eligible_team_members(
	...
	*,
	...
	sale_rows = [
		row for row in memberships if row.get("parent") and row.get("function") in (None, "", "Sale")
	]
...
def retry_student_routing(request_name: str) -> dict[str, Any]:
	...
	student = canonical_student(request.student) or request.student
	student = frappe.get_doc("CRM Student", student)
