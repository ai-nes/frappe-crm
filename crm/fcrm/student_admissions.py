...
def _canonical_student_reference(value: Any) -> str:
	student = canonical_student(str(value or "").strip())
	if student:
		return student
	_fail("NOT_FOUND", _("The Student does not exist."))
...
def _actor_scope(student: str, action: str) -> dict[str, Any]:
	...
	student = _canonical_student_reference(student)
	...
	if (
		...
		and required not in capabilities
		and "admissions.oversee" not in capabilities
	):
...
def _interaction_type(preferred: str = "MESSAGE") -> str:
	...
	name = frappe.db.get_value(
		"CRM Interaction Type", {"enabled": 1}, "name", order_by="sort_order asc, name asc"
	)
	if not name:
		name = (
			frappe.get_doc({"doctype": "CRM Interaction Type", "code": preferred, "display_name": preferred})
			.insert(ignore_permissions=True)
			.name
		)
	return name
...
def _call(student_doc, actor: str, data: dict[str, Any], *, success: bool, idempotency_key: str):
	...
	call = frappe.get_doc(
		{
			...
			"from": _text(
				frappe.db.get_value("User", actor, "mobile_no")
				or frappe.db.get_value("User", actor, "phone")
				or "Admissions"
			),
	...
	interaction = frappe.db.get_value(
		"CRM Interaction", {"reference_doctype": "Call Log", "reference_docname": call.name}, "name"
	)
	if not interaction:
...
def _communication(student_doc, actor: str, data: dict[str, Any]):
	...
	interaction = frappe.db.get_value(
		"CRM Interaction", {"reference_doctype": "Communication", "reference_docname": comm.name}, "name"
	)
	if not interaction:
		from crm.fcrm.interaction_log import create_interaction
...
def _event_action(
	student: str, actor: str, action: str, data: dict[str, Any], idempotency_key: str, correlation_id: str
):
	...
	frappe.flags.student_admissions_scope = {
		"actor": actor,
		"student": student,
	...
	try:
		...
		if action == "event_invite":
			...
			interaction = _insert_interaction(
				student,
				actor,
				_("Invited to {0}").format(event_name),
				data.get("notes"),
				interaction_type="EVENT_PARTICIPATION",
				external_id=f"admissions:{idempotency_key}:interaction",
			)
			result = {"interaction": interaction}
			if data.get("campaign"):
				result["campaign"] = student_attribution.record_campaign_touchpoint(
					student=student,
					crm_campaign=data["campaign"],
					source="Manual",
					notes=_("Open Day invitation"),
		...
		if action == "event_checkin":
			...
			supersedes = active
		event = student_attribution.record_event_participation(
			student=student,
			crm_event=event_name,
			status=status,
			checked_in_at=frappe.utils.now_datetime() if status == "Checked-in" else None,
			supersedes=supersedes,
			idempotency_key=f"{idempotency_key}:event",
			correlation_id=correlation_id,
		)
		summary = (
			_("Checked-in at {0}").format(event_name)
			if status == "Checked-in"
			else _("Registered for {0}").format(event_name)
		)
		interaction = _insert_interaction(
			student,
			actor,
...
def _structured_note(student_doc, actor: str, data: dict[str, Any]):
	...
	if isinstance(content, dict):
		content = "\n".join(
			f"<p><strong>{_text(key)}</strong>: {frappe.utils.escape_html(str(value))}</p>"
...
def _dispatch(
	student_doc, actor: str, action: str, data: dict[str, Any], idempotency_key: str, correlation_id: str
):
	if action == "digital_signal":
		...
		return {
			"interaction": _insert_interaction(
				...
				signal,
				data.get("notes"),
				external_id=f"admissions:{idempotency_key}",
			)
		}
	...
	if action == "lifecycle_transition":
		...
		return request_transition(
			student=student_doc.name,
			target_stage=_required(data.get("to_stage"), "to_stage"),
			reason=data.get("reason"),
			expected_revision=int(student_doc.get("lifecycle_revision") or 0),
			idempotency_key=f"{idempotency_key}:lifecycle",
			correlation_id=correlation_id,
		)
	...
	if action == "create_task":
		task = frappe.get_doc(
			{
				"doctype": "Task",
				"title": _required(data.get("title"), "title"),
				"description": data.get("description"),
				"priority": data.get("priority") or "Medium",
				"status": "Todo",
				"assigned_to": data.get("assigned_to") or actor,
	...
	if action == "assign_counselor":
		...
		if not team:
			memberships = frappe.get_all(
				"CRM Team Membership",
				filters={"parent": counselor, "parenttype": "CRM Staff"},
				fields=["team"],
		...
		return change_student_ownership(
			...
			target_id=counselor,
			target_team_id=team,
			reason=data.get("reason") or _("Admissions assignment"),
			idempotency_key=f"{idempotency_key}:ownership",
			expected_revision=int(student_doc.get("ownership_revision") or 0),
			correlation_id=correlation_id,
		)
...
def perform_action(
	*,
	...
	fingerprint = _fingerprint(
		{"student": student, "action": action, "expected_revision": str(expected_revision), "payload": data}
	)
	...
	try:
		...
		frappe.db.set_value(
			"CRM Student", student, "engagement_revision", new_revision, update_modified=False
		)
		result.update(
			{
				"status": "created",
		...
		if isinstance(exc, StudentAdmissionsError):
			_finish_receipt(
				receipt,
				{"status": "failed", "student": student, "action": action},
				outcome="failed",
				error_code=exc.code,
			)
			raise
...
def set_attachment_visibility(
	*, student: str, file_name: str, is_private: Any, confirm_public: bool = False
) -> dict[str, Any]:
	student = _canonical_student_reference(_required(student, "student"))
	file_name = _required(file_name, "file_name")
	scope = _actor_scope(student, "create_insight_note")
	if not (
		scope["actor"] == "Administrator"
		or "System Manager" in scope["roles"]
		or "admissions.oversee" in scope["capabilities"]
	):
