...
def _new_receipt(kind, actor, student, key, fingerprint, scope, correlation_id):
	...
	return frappe.get_doc({"doctype": RECEIPT, "receipt_key": command_key, "command_key": command_key,
		"command_kind": kind if kind in {"recommendation_decision", "student_task_decision", "sales_action", "sales_action_reassign"} else "interaction_outcome",
...
def _event(kind, student, recommendation, action, actor, scope, receipt, correlation_id, revision, payload, student_task=None):
	...
	event_type = "recommendation_decided" if kind.startswith("recommendation.") else "task_decided" if kind.startswith("student_task.") else {
	...
	values = {"doctype": DECISION_EVENT, "event_id": frappe.generate_hash(length=20), "student": student,
		"recommendation": recommendation, "student_task": student_task, "sales_action": action, "event_type": event_type, "actor": actor,
...
def decide_student_task(name: str, expected_revision: Any, status: str, idempotency_key: str, correlation_id: str | None = None, decision_reason: str | None = None, due_at: Any = None, assignee_staff: str | None = None, revisit_at: Any = None, defer_kind: str | None = None, expected_modified: str | None = None):
	...
	try:
		...
		action = None
		if status == "accepted":
			action = frappe.db.get_value(ACTION, {"student_task": doc.name}, "name")
			if not action:
				action_doc = frappe.get_doc({"doctype": ACTION, "student_task": doc.name, "student": doc.student, "action_type": doc.action_type, "created_at": now_datetime(), "due_at": due_at, "assignee_staff": assignee_staff, "assignee_snapshot": {"assignee": assignee_staff, "student": doc.student}, "assignment_history": json.dumps([]), "assignment_revision": 1, "action_revision": 1, "correlation_id": correlation_id, "idempotency_key": key, "execution_status": "planned"})
				action_doc.flags.from_phase6_command = True; action_doc.insert(ignore_permissions=True); action = action_doc.name
				doc.db_set("sales_action", action, update_modified=False)
		...
		event = _event(f"student_task.{status}", doc.student, None, action, actor, scope, receipt, correlation_id, doc.decision_revision, {"status": status, "from_state": previous_state, "reason": decision_reason, "revisit_at": str(revisit_at) if revisit_at else None}, student_task=doc.name)
		result = {"status": status, "task": doc.name, "sales_action": action, "revision": doc.decision_revision, "event": event.name, "receipt": receipt.name}; _finish(receipt, result); return result
	finally:
		frappe.flags.phase6_decision_command = previous_flag
		frappe.flags.student_task_command = previous_task_flag
...
def reassign_sales_action(name: str, expected_revision: Any, assignee_staff: str, idempotency_key: str, reason: str, correlation_id: str | None = None, _internal_service: bool = False):
	...
	try:
		...
		try:
			history = json.loads(action.get("assignment_history") or "[]")
		except (TypeError, ValueError):
			history = []
		action.assignment_history = json.dumps([*history, {"from": previous_assignee, "to": assignee_staff, "actor": actor, "reason": reason, "at": str(now_datetime())}])
