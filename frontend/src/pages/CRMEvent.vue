  ...
  <div v-if="doc.name" class="flex h-full overflow-hidden">
    ...
    <Resizer class="flex flex-col justify-between border-l" side="right">
      <div
        ...
        @click="copyToClipboard(crmEventId)"
      >
        {{ __(doc.title || crmEventId) }}
      </div>
      <div
        v-if="sections.data"
...
const { doctypeMeta } = getMeta('CRM Event')

const props = defineProps({
  crmEventId: { type: String, required: true },
})
...
const { document, error } = useDocument('CRM Event', props.crmEventId)
...
const title = computed(() => {
  ...
  return doc.value?.[t] || props.crmEventId
})
