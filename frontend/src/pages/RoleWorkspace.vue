  ...
  >
    <LayoutHeader>
      <template #left-header>
        <ViewBreadcrumbs
          :routeName="response.title || workspace.title || route.name"
        />
      </template>
    ...
    >
      <div class="mx-auto max-w-7xl space-y-5">
        ...
        <section v-else-if="state === 'denied'" class="state-card" role="alert">
          <FeatherIcon
            name="shield-off"
        ...
        >
          ...
          <p class="text-sm text-ink-gray-6">
            {{
              response.message ||
              __(
                'Không gian làm việc này chưa khả dụng trong môi trường hiện tại.',
              )
            }}
          </p>
        </section>

        <section v-else-if="state === 'error'" class="state-card" role="alert">
          <FeatherIcon
            name="alert-triangle"
        ...
        <section v-else-if="state === 'empty'" class="state-card" role="status">
          <FeatherIcon
            name="inbox"
            class="size-8 text-ink-gray-4 mb-2 opacity-60"
          />
          <h2 class="text-base font-semibold text-ink-gray-9">
            {{ response.emptyTitle || __('Chưa có dữ liệu') }}
          </h2>
          <p class="text-sm text-ink-gray-6">
            {{
              response.emptyMessage ||
              __('Hãy thử thay đổi bộ lọc hoặc quay lại khi có dữ liệu mới.')
            }}
          </p>
        </section>
        ...
        <template v-else>
          ...
          >
            ...
            <span>{{
              __(
                'Đang hiển thị bản chụp trước đó trong khi tải dữ liệu mới nhất.',
              )
            }}</span>
          </div>
          ...
          >
            <FeatherIcon
              name="alert-triangle"
              class="size-4.5 shrink-0 text-amber-600 mt-0.5"
            />
          ...
          <div
            class="grid gap-5"
            :class="
              response.sidePanel ? 'xl:grid-cols-[minmax(0,1fr)_20rem]' : ''
            "
          >
            <StudentWorkTable
            ...
            >
              <h2 class="font-semibold text-ink-gray-9">
                {{ response.sidePanel.title }}
              </h2>
              <p class="mt-2 text-sm text-ink-gray-6">
                {{ response.sidePanel.description }}
              </p>
            </aside>
          </div>
        </template>
...
export function normalizeWorkspaceFilters(schema = [], query = {}) {
  return normalizeWorkspaceFilterSchema(schema).reduce((filters, field) => {
    ...
    if (
      typeof value === 'string' &&
      value &&
      (!allowed.length || allowed.includes(value))
    )
...
export function getWorkspaceFilterSchema(response = {}, workspace = {}) {
  ...
  if (Array.isArray(response.filterSchema?.allowed))
    return normalizeWorkspaceFilterSchema(response.filterSchema.allowed)
  ...
  return schema
    .map((field) => {
      if (typeof field === 'string')
        return { key: field, label: filterLabel(field), options: [] }
      if (!field?.key) return null
      return {
        ...field,
        label: field.label || filterLabel(field.key),
...
export function workspaceRequest(
  ...
  const preset =
    workspace.preset && typeof workspace.preset === 'object'
      ? workspace.preset
      : {}
...
export function normalizeWorkspaceResponse(summary = {}, rows, series) {
  ...
  const readyChannels = [rows, series].filter(
    (result) => result?.contractStatus === 'ready',
  )
  if (readyChannels.some((result) => !sameSnapshotMetadata(summary, result))) {
    return {
      ...
      state: 'unavailable',
      message: __(
        'The workspace data changed while it was loading. Please refresh.',
      ),
      snapshot: null,
    }
  }
  ...
  const partialSources = [summary, rows, series].filter(
    (result) =>
      result &&
      (['partial', 'unavailable', 'migration_required'].includes(
  ...
  if (partialSources.length) {
    ...
    response.partialMessage = partialSources
      .map(
        (result) =>
          result.availability?.reason ||
          result.explanation ||
          __('Some workspace data is not available for this view.'),
      )
  ...
  const rowColumns = Array.isArray(rows?.columns)
    ...
    : Array.isArray(rows?.rowSchema)
      ? rows.rowSchema.map((field) => ({
          key: field.field || field.key,
          label: field.label || field.field || field.key,
  ...
  if (Array.isArray(series?.series)) {
    response.analytics = {
      ...
      sourceUpdatedAt:
        series.sourceUpdatedAt ||
        series.snapshotContext?.asOf ||
        summary.snapshotContext?.asOf,
...
export function sameSnapshotMetadata(summary = {}, result) {
  ...
  return (
    (!expected.definitionVersion ||
      expected.definitionVersion === actual.definitionVersion) &&
    (!expected.timezone || expected.timezone === actual.timezone) &&
    (!expected.asOf || expected.asOf === actual.asOf)
  )
}

export async function resolveWorkspaceDetail(
  target,
  request,
  navigate,
  context = {},
  ...
  const resolved = await request(target.resolver, {
    ...context,
    token: target.token,
  })
...
import { Button, call, FeatherIcon, usePageMeta } from 'frappe-ui'
...
const response = ref({})
const filters = ref(
  normalizeWorkspaceFilters(props.workspace.filters, route.query),
)
let controller
const hasData = computed(() =>
  Boolean(
    response.value.kpis?.length ||
...
const state = computed(() =>
  errorMessage.value ? 'error' : workspaceState(response.value),
)
...
watch(
  () => [props.workspace, route.query],
  () => {
    filters.value = normalizeWorkspaceFilters(
      activeFilterSchema.value,
      route.query,
    )
    refresh()
  },
...
async function refresh(cursor) {
  ...
  try {
    ...
    if (error?.name !== 'AbortError')
      errorMessage.value = error?.message || __('Please try again.')
  } finally {
...
function throwIfAborted(signal) {
  if (signal?.aborted)
    throw new DOMException('Workspace request was aborted.', 'AbortError')
}
...
async function readRoleWorkspace({ workspace, filters, cursor, signal }) {
  ...
  if (!summaryArgs) {
    return {
      state: 'unavailable',
      message: __('This workspace route is not configured.'),
    }
  }
  ...
  if (!['ready', 'partial'].includes(summary?.contractStatus))
    return normalizeWorkspaceResponse(summary)
  if (!summary.snapshot) {
    return {
      state: 'unavailable',
      contractStatus: 'unavailable',
  ...
  const signedArgs = workspaceRequest(
    workspace,
    filters,
    summary.snapshot,
    cursor,
...
async function openStudent(row) {
  ...
  try {
    const context = workspaceRequest(
      props.workspace,
      filters.value,
      response.value.snapshot,
    )
    const navigated = await resolveWorkspaceDetail(
      target,
      call,
...
function loadMore() {
  refresh(response.value.students?.nextCursor)
}
function drillDown(item) {
  if (item?.drilldown?.route) router.push(item.drilldown.route)
}
function handleActionError(error) {
  errorMessage.value =
    error?.message || __('The action could not be completed.')
}
async function exportRows() {
  ...
  try {
    const result = await call('crm.api.role_workspaces.get_workspace_export', {
      ...
      export_id: exportAction.token,
    })
    if (!result?.download)
      throw new Error(
        result?.exportReason || __('Export is not available for this view.'),
      )
