<template>
  ...
  <div v-if="doc.name" class="flex h-full overflow-hidden">
    ...
    <Resizer class="flex flex-col justify-between border-l" side="right">
      ...
      >
        {{ __(doc.school_name || highSchoolId) }}
      </div>
      <div
        v-if="sections.data"
...
const { doctypeMeta } = getMeta('CRM High School')
...
const breadcrumbs = computed(() => {
  let items = [{ label: __('High Schools'), route: { name: 'High Schools' } }]
  items.push({
    label: doc.value?.school_name || props.highSchoolId,
    route: { name: 'High School', params: { highSchoolId: props.highSchoolId } },
  })
  return items
})
...
const title = computed(() => {
  ...
  return doc.value?.[t] || props.highSchoolId
})
...
const { tabIndex } = useActiveTabManager(tabs, 'lastHighSchoolTab')

const sections = createResource({
  url: 'crm.fcrm.doctype.crm_fields_layout.crm_fields_layout.get_sidepanel_sections',
  cache: ['sidePanelSections', 'CRM High School'],
  params: { doctype: 'CRM High School' },
  auto: true,
})
</script>
