        ...
        <section class="recommendations-section mt-6" aria-labelledby="recommended-actions-title">
          ...
          <div v-if="brief.recommended_actions?.length" class="mt-3 space-y-3">
            <article v-for="item in brief.recommended_actions" :key="item.action || item" class="recommendation-row rounded-md px-4 py-4">
              ...
              <button v-if="item.based_on?.length" type="button" class="mt-2 text-left text-xs text-ink-blue-6 hover:underline focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2" @click="inspectEvidence(item.based_on)">{{ __('Based on {0} evidence references', [item.based_on.length]) }}</button>
