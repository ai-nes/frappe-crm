  ...
  >
    <div class="mb-4">
      ...
      <p class="mt-1 text-sm text-ink-gray-5">
        {{
          __(
            'Campaign, event and scholarship signals for the next conversation.',
          )
        }}
      </p>
    </div>

    <div v-if="loading && !context?.hasContent" role="status" class="space-y-3">
      <div class="h-4 w-48 animate-pulse rounded bg-surface-gray-2" />
      <div class="grid gap-3 sm:grid-cols-2">
        <div
          v-for="item in 2"
          :key="item"
          class="h-12 animate-pulse rounded bg-surface-gray-1"
        />
      </div>
      <span class="sr-only">{{ __('Loading admissions context…') }}</span>
    </div>
    <div v-else>
      <p v-if="loading" role="status" class="mb-3 text-sm text-ink-gray-5">
        {{ __('Refreshing admissions context…') }}
      </p>
      <p v-if="!context?.hasContent" class="text-sm text-ink-gray-5">
        {{
          __(
            'No campaign, event, scholarship, or next-action context is available yet.',
          )
        }}
      </p>
      <dl v-else class="grid gap-4 sm:grid-cols-2">
        <div v-if="context.campaign" class="break-words">
          ...
          <dd class="mt-1 text-sm text-ink-gray-8">
            ...
            <span v-if="context.campaign.source" class="text-ink-gray-5">
              · {{ __(context.campaign.source) }}</span
            >
            <time
        ...
        <div v-if="context.scholarship" class="break-words">
          <dt class="text-sm text-ink-gray-5">
            {{ __('Scholarship interest') }}
          </dt>
          <dd class="mt-1 text-sm text-ink-gray-8">
            <span class="font-medium">{{ context.scholarship.label }}</span>
            <span v-if="context.scholarship.notes" class="block">{{
              context.scholarship.notes
            }}</span>
            <span v-if="scholarshipMeta" class="mt-1 block text-ink-gray-5">{{
              scholarshipMeta
            }}</span>
          </dd>
        </div>
        ...
        <div
          v-if="context.score"
          class="break-words sm:col-span-2 border-t border-outline-gray-2 pt-3"
        >
          <dt class="text-sm text-ink-gray-5">
            {{ __('Potential score freshness') }}
          </dt>
          <dd
            class="mt-1 flex flex-wrap items-center gap-2 text-sm text-ink-gray-8"
          >
            <span v-if="context.score.latest !== null">{{
              __('Latest score: {0}', [context.score.latest])
            }}</span>
            <Badge
              :label="
                context.score.state === 'current'
                  ? __('Up to date')
                  : context.score.state === 'pending'
                    ? __('Recalculation pending')
...
const eventTheme = computed(() => {
  ...
  if (['invited', 'registered'].includes(props.context?.event?.status))
    return 'blue'
  return 'gray'
})
...
const scholarshipMeta = computed(() => {
  ...
  if (
    scholarship.confidence !== null &&
    scholarship.confidence !== undefined &&
    scholarship.confidence !== ''
  ) {
