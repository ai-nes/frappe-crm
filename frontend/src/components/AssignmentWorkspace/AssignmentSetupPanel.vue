<template>
  <section class="flex flex-col gap-5" data-testid="assignment-setup-panel">
    <div v-if="!workflowOnly" class="flex flex-wrap items-start justify-between gap-3">
      <div>
    ...
    <template v-else>
      <div v-if="!workflowOnly" class="grid gap-4 sm:grid-cols-3">
        <div
        ...
        <div class="flex items-center justify-between gap-3 p-4">
          ...
          <div class="flex shrink-0 items-center gap-2">
            ...
            <button
              ...
              "
              :title="sections.staff ? __('Thu gọn Nhân sự') : __('Mở Nhân sự')"
        ...
        <div class="flex items-center justify-between gap-3 p-4">
          <div>
            <span>
              ...
              <span class="mt-1 block text-xs text-ink-gray-5">{{
                __('Xác định nhóm phụ trách từng khu vực.')
              }}</span>
            </span>
          </div>
          <button
            type="button"
            class="inline-flex min-h-9 min-w-9 shrink-0 items-center justify-center rounded-md text-ink-gray-5 hover:bg-surface-gray-2 focus:outline-none focus:ring-2 focus:ring-outline-gray-4"
            :aria-expanded="sections.zone"
            :aria-label="
              sections.zone ? __('Thu gọn khu vực') : __('Mở khu vực')
            "
            :title="sections.zone ? __('Thu gọn khu vực') : __('Mở khu vực')"
...
const sections = reactive({ zone: true, team: false, staff: false })
